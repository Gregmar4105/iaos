...
import { ArrowLeftIcon, CalendarIcon, MapPinIcon, PlaneTakeoff } from 'lucide-react';
...
interface DeparturesProps extends SharedData {
...
export default function Departures({ flights, currentPage, lastPage, total }: DeparturesProps) {
    const handlePageChange = (page: number) => {
        router.visit(`/departures?page=${page}`, {
            preserveState: true,
            preserveScroll: true,
        });
    };
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-600">Showing {flights.length} of {total} departure flights</p>
                            </div>
                        </div>
                        <PlaneTakeoff className="h-8 w-8 text-orange-500" />
                    </div>
                                ...
                                <div key={flight.id} className="px-6 py-5 hover:bg-slate-50 transition-colors">
                                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-center">
                                        ...
                                        <div className="lg:col-span-3">
                                            <p className="text-xs text-slate-500 mb-1">Scheduled Departure</p>
                                            <div className="flex items-center gap-2">
                                                <CalendarIcon className="h-4 w-4 text-slate-400" />
                                                <span className="font-semibold text-slate-900 text-sm">{formatDate(flight.scheduled_departure_time)}</span>
                                            </div>
                                        </div>
